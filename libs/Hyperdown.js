	            ...
	            for (var key in lines) {
	                ...
	                if (codeMatches) {
	                    if (this.isBlock('code')) {
	                        ...
	                        var _isAfterList = false;
	                        if (this.isBlock('list')) {
	                            var space = block[3];

	                            _isAfterList = space > 0 && codeMatches[1].length >= space || codeMatches[1].length > space;
	                        }
	                        this.startBlock('code', key, [codeMatches[1], codeMatches[3], _isAfterList]);
	                    }
	                    continue;
	                ...
	                if (htmlMatches1) {
	                    ...
	                    var _tag = htmlMatches2[1].toLowerCase();

	                    if (this.isBlock('html', _tag)) {
	                        this.setBlock(key).endBlock();
	                    }
	        ...
	        value: function parseList(lines) {
	            ...
	            rows.forEach(function (row) {
	                if (Array.isArray(row)) {
	                    ...
	                    var _pattern = new RegExp("^\s{" + secondMinSpace + "}");
	                    leftLines.push(row.replace(_pattern, ''));
	                }
	            });
